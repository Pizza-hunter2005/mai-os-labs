    ...
    public:
        ...
        std::string create(int child_id) {
            ...
            if (left_id == -2) {
                ...
                std::string fail = "Error: " + std::to_string(child_id) + "can not create the calculation node\n";
                return fail;
            }
            int fork_id = fork();
            if (fork_id == 0) {
                if (execl("./client", "client",  std::to_string(child_id).c_str(),  std::to_string(port).c_str(),  std::to_string(id).c_str(), (char*)NULL) == -1) {
                    std::cout << "Error: " + std::to_string(child_id) + " can not run the execl-command" <<  std::endl;
                    exit(EXIT_FAILURE);
                }
            }
            else {
                ...
                try {
                    if (isleft) {
                        left.set(zmq::sockopt::sndtimeo, 3000);
                        send_message(left, "pid");
                        child_pid = receive_message(left);
                    }
                    else {
                        right.set(zmq::sockopt::sndtimeo, 3000);
                    ...
                    std::string fail = "Error: " + child_pid + " can not connect to the child";
                    return fail;
                }
            }
            return ("Error: " + std::to_string(child_id) + ": Unexpected error occurred");
        }
        ...
        std::string sendstring (std::string string, int id) {
            std::string answer = "Error: " + std::to_string(id) +  ": Parent not found";
            if (left_id == -2 && right_id == -2) {
                ...
                if (ping(left_id) == "Ok: 1") {
                    ...
                    try {
                        ...
                        message = "Error: " +  std::to_string(left_id) + ": Parent not found";
                    }
                    if (message != "Error: " +  std::to_string(left_id) + ": Parent not found") {
                        answer = message;
                    }
                }
                if (ping(right_id) == "Ok: 1") {
                    ...
                    try {
                        ...
                        message = "Error: " +  std::to_string(right_id) + ": Parent not found";
                    }
                    if (message != "Error: " +  std::to_string(right_id) + ": Parent not found") {
                        answer = message;
                    }
                }
            }
